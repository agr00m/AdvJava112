...
public class WebClient extends CloseableFrame implements Runnable, Interruptible, ActionListener {
    ...
    public WebClient(String title) {
        ...
        int fontSize = 14;
        Font labelFont = new Font("Serif", Font.BOLD, fontSize);
        Font headingFont = new Font("SansSerif", Font.BOLD, fontSize+4);
        Font textFont = new Font("Monospaced", Font.BOLD, fontSize-2);
        Panel inputPanel = new Panel();
        inputPanel.setLayout(new BorderLayout());
        Panel labelPanel = new Panel();
        labelPanel.setLayout(new GridLayout(4,1));
        hostField = new LabeledTextField("Host:", labelFont, 30, textFont);
    ...
    public void run() {
        isInterrupted = false;
        if (hasLegalArgs()) {
            new HttpClient(host, port, requestLine, requestHeaders, resultArea, this);
        }
    }
